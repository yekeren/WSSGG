...
class WSSceneGraphV3(model_base.ModelBase):
  """WSSceneGraphV3 model to provide instance-level annotations. """

  def __init__(self, options, is_training):
    """Constructs the WSSceneGraphV3 instance. """
    super(WSSceneGraphV3, self).__init__(options, is_training)

    if not isinstance(options, model_pb2.WSSceneGraphV3):
      raise ValueError('Options has to be an WSSceneGraphV3 proto.')
    ...
    if not self.is_training:
      ...
      (accum_probs, sub_ids, obj_ids,
       pred_ids) = self._beam_search_refine_triple(
           search.n_triple,
           search.get_subject_feature(shared_hiddens),
           search.get_object_feature(shared_hiddens),
      ...
      (n_valid_example, scores, sub, sub_box_index, pred, obj,
       obj_box_index) = utils.beam_search_post_process(
           search.n_triple,
           search.subject_proposal_index,
           search.object_proposal_index,
           accum_probs,
           sub_ids,
           pred_ids,
           obj_ids,
      ...
      predictions.update({
          ...
          'beam/accum_probs':
              scores,
          'beam/subject/box':
              search._gather_proposal_by_index(proposals, sub_box_index),
          'beam/object/box':
              search._gather_proposal_by_index(proposals, obj_box_index),
          'beam/subject':
              self.id2entity(sub),
          'beam/object':
              self.id2entity(obj),
  ...
  def _compute_triple_refine_losses(self,
                                    n_triple,
                                    subject_feature,
                                    object_feature,
                                    subject_ids,
    ...
    # Create visual sequence features.
    # - visual_seq_feature = [batch, max_n_triple, 3, visual_dims].
    relation_feature = tf.add(subject_feature, object_feature)
  ...
  def build_losses(self, inputs, predictions, **kwargs):
    ...
    # Build RNN to contextualizing label generation process, using the last MPS.
